...
import {art,loadArt,spriteElement,drawWorld,thumbnail} from './art.js';
...
let house=starterHouse(),unlocked=starterUnlocks(),category='garden',selected=null,selectedPart=null,ready=false,storageAvailable=true;
...
const positive=['すてき！','かわいいね！','いいかんじ！','おいしそう！'];
...
function changed(message=true){persistDraft();render();if(message){const text=positive[Math.floor(Math.random()*positive.length)];$('guide-text').textContent=text;audio.effect();audio.say(text);}}
function burst(x,y){for(let i=0;i<16;i++)particles.push({x,y,vx:(Math.random()-.5)*130,vy:-30-Math.random()*110,life:1,size:3+Math.random()*5,color:['#e0a2b0','#dfbc72','#a6c4a3','#fff5d6'][i%4]});render();}
function resize(){
...
function paint(time){
  ...
  if(art.background)ctx.drawImage(art.background,0,0,camera.width,camera.height);
  ctx.translate(camera.x,camera.y);ctx.scale(camera.s,camera.s);drawWorld(ctx,house,selected,particles,false);
...
function addPart(id,x,y){const p=partById(id);if(!p||!unlocked.includes(id))return;remember();const pos=position(p,x,y),item={id:uid(),part:id,...pos};house.items.push(item);selected=item.id;burst(pos.x,pos.y);changed();}
...
function makeParts(){
  ...
  for(const p of PARTS.filter(p=>p.category===category)){
    ...
    if(locked){const badge=document.createElement('span');badge.className='part-badge';badge.textContent='👩‍🍳';b.append(badge);b.onclick=()=>{if(ready)mini.open(p.game);};}
    else{
...
async function saveCurrent(showFeedback=true){
  try{const record={...structuredClone(house),savedAt:Date.now(),thumbnail:thumbnail(house)};await saveHouse(record);house.savedAt=record.savedAt;storageAvailable=true;await setState('draft',house);$('save-status').textContent='アルバムに ほぞんしたよ';if(showFeedback){toast('アルバムに はいったよ！');audio.effect('win');audio.say('できたね！');}return true;}catch(e){storageError(e);return false;}
}
...
function makeGames(){
  $('game-cards').replaceChildren();for(const [i,g] of GAMES.entries()){
    const b=document.createElement('button');b.className='game-card';b.append(spriteElement([0,1,5,13][i],'game-art'));const h=document.createElement('h3');h.textContent=g.name;const small=document.createElement('small');small.textContent=['かべが ふえるよ','やねが ふえるよ','かざりが ふえるよ','おうちの だいが ふえるよ'][i];const arrow=document.createElement('span');arrow.className='game-arrow';arrow.textContent='▶';arrow.setAttribute('aria-hidden','true');b.append(h,small,arrow);b.onclick=()=>mini.open(g.id);$('game-cards').append(b);
  }
}
async function finishGame(id){
  ...
  $('reward-message').textContent=reward.fresh.length?'あたらしいおかしが ふえたよ！':'おいしいおかしが できたよ！';$('reward-parts').replaceChildren(...ids.map(id=>spriteElement(partById(id).sprite)));
...
async function loadGallery(){
  ...
  try{
    ...
    for(const [i,record] of all.entries()){
      const b=document.createElement('button');b.className='gallery-card';b.setAttribute('aria-label',`おうち ${all.length-i}をひらく`);const img=document.createElement('img');img.src=record.thumbnail||thumbnail(record);img.alt=`おかしのおうち ${all.length-i}`;img.loading='lazy';const p=document.createElement('p');p.textContent=`おうち ${all.length-i}`;const date=document.createElement('span');date.textContent=new Date(record.savedAt).toLocaleDateString('ja-JP',{month:'long',day:'numeric'});p.append(date);b.append(img,p);
